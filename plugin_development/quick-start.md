...
1. **打开** `__init__.py` 文件 (用文本编辑器)

2. **找到这一行**:
   ```python
   greeting = await self.config.get("my_settings.greeting", default="Hello")
   ```

3. **修改为**:
   ```python
   greeting = await self.config.get("my_settings.greeting", default="你好")
   ```
...
1. 在服务器命令行窗口按 `Ctrl + C`
2. 等待服务器停止
3. 重新运行: `python -m plugin.user_plugin_server`

**方法 B: 只重载插件** (推荐)

使用浏览器访问:
```
http://127.0.0.1:48912/plugin/hello_world/reload
```

或使用命令行:
```bash
# Windows
Invoke-WebRequest -Uri "http://127.0.0.1:48912/plugin/hello_world/reload" -Method POST

# Linux / macOS
curl -X POST http://127.0.0.1:48912/plugin/hello_world/reload
```

### 验证修改

再次调用插件,应该看到新的问候语 "你好, Alice!"

---

## 第六步: 添加依赖 (可选)

如果插件需要额外的 Python 包,使用 `pyproject.toml` 管理依赖:

### 创建 pyproject.toml

**步骤**:

1. **在 `hello_world` 文件夹中创建新文件**
   - 文件名: `pyproject.toml`
...
# 初始化 poetry (如果还没有 pyproject.toml)
poetry init
...
1. **学习高级功能**:
   - [插件间通信](./inter-plugin-communication.md)
   - [Run Protocol 集成](./run-protocol-integration.md)
   - [数据存储](./data-storage.md)

2. **查看示例插件**:
   - `plugins/testPlugin/`: 功能完整的示例
   - `plugins/load_tester/`: 性能测试示例
